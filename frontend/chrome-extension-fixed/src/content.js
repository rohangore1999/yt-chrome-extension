...
const saveVideoId = () => {
  ...
  if (videoId) {
    ...
    chrome.storage.sync.set({ videoId: videoId, lastVideoId: videoId }, () => {
...
const togglePopup = async () => {
  ...
  if (popupOverlay.style.display === "none" || !popupOverlay.style.display) {
    ...
    if (currentVideoId) {
      // Get stored video ID from Chrome storage
      chrome.storage.sync.get(["lastVideoId"], (result) => {
        const storedVideoId = result.lastVideoId;

        // Check if video ID has changed
        if (currentVideoId !== storedVideoId) {
          // Store new video ID in Chrome storage
